  ...
  async (email, productId, quantity,) => {
    ...
    if (cart) {
      ...
      if (indexFound !== -1 && quantity <= 0) {
        ...
        cart.totalPrice += priceDiscount * quantity;
      } else if (quantity > 0) {
        ...
        cart.items.push({
          ...
          totalProductPrice: priceDiscount * quantity
        });
    ...
    for (const indexFound of indexesFound) {
      if (
        ...
        const updatedProductTotalPrice =
          cart.items[indexFound].totalProductPrice - priceDiscount;
        const updatedCartTotalQuantity = cart.totalQuantity - 1;
        const updatedCartTotalPrice = cart.totalPrice - priceDiscount;
